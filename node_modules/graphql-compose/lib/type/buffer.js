...
exports.default = new graphql_1.GraphQLScalarType({
    ...
    parseLiteral(ast) {
        if (ast.kind !== graphql_1.Kind.STRING) {
            throw new graphql_1.GraphQLError(`Query error: Can only parse strings to buffers but got a: ${ast.kind}`, graphqlVersion_1.graphqlVersion >= 17 ? { nodes: [ast] } : [ast]);
        }
        const result = Buffer.from(ast.value);
        if (ast.value !== result.toString()) {
            throw new graphql_1.GraphQLError('Query error: Invalid buffer encoding', graphqlVersion_1.graphqlVersion >= 17 ? { nodes: [ast] } : [ast]);
        }
        return result;
    },
});
