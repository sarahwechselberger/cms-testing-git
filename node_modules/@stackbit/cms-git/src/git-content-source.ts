...
export class GitContentSource implements ContentSourceInterface<unknown, unknown, DocumentContext, AssetContext> {
    ...
    private localDevSync?: LocalDevSyncOptions;
    private syncRemote!: boolean;

    // internal flag which shows that current class (and the one which inherits it) is aimed to work with files within the repo
    protected isGitCms = true;

    constructor(options: GitContentSourceOptions) {
        this.fileSystemContentSource = new FileSystemContentSource(options);
        this.rootPath = options.rootPath;
        this.projectUrl = '';
        this.localDevSync = options.localDevSync;
    }
    ...
    async init(options: InitOptions<unknown, DocumentContext, AssetContext>): Promise<void> {
        ...
        this.syncRemote = !this.localDev;

        if (this.localDev && this.localDevSync && this.localDevSync.repoUrl && this.localDevSync.repoWorkingBranch && this.localDevSync.repoPublishBranch) {
            this.git.setRepoUrl(this.localDevSync.repoUrl);
            this.git.setRepoBranch(this.localDevSync.repoWorkingBranch);
            this.git.setRepoPublishBranch(this.localDevSync.repoPublishBranch);
    ...
    async onFilesChange?({
        ...
        if (!this.syncRemote || !result.contentChanges) {
            return result;
        }
        const changesMap = await this.getChangesMap();
        return {
    ...
    getAssetFilePath(asset: Asset): string | undefined {
        const assetField = asset.fields.file;
        const relFilePath = assetField?.localized === true ? assetField.locales[Object.keys(assetField.locales)[0] ?? '']?.url : assetField?.url;
        if (!relFilePath) {
            return relFilePath;
        }
    ...
    async updateDocument(options: { document: Document<DocumentContext>; operations: UpdateOperation[]; userContext?: User }): Promise<void> {
        ...
        if (this.syncRemote) {
            ...
            if (document.context.filePath !== updatedDocument.context.filePath) {
                ...
                // Delete the previous document only if its id is different.
                // When using file-ids, the id remains the same when copying the file.
    ...
    private async getChangesMap() {
        ...
        for (const commitEntry of entries) {
            for (const { status, filePath, fromFilePath } of commitEntry.changes) {
                ...
                } else if (fromFilePath && changesMap[fromFilePath] && !diff.includes(fromFilePath)) {
                    const prevStatus = changesMap[fromFilePath]!.status;
                    changesMap[filePath] = {
                        updatedBy: commitEntry.author === DEFAULT_AUTHOR_EMAIL ? [] : [commitEntry.author],
                        updatedAt: commitEntry.timestamp.toISOString(),
                        status: diff.includes(filePath) ? (prevStatus === 'added' ? 'added' : status) : 'published',
                        fromFilePath: changesMap[fromFilePath]!.fromFilePath ?? fromFilePath
                    };
                    delete changesMap[fromFilePath];
                } else {
                    changesMap[filePath] = {
                        updatedBy: commitEntry.author === DEFAULT_AUTHOR_EMAIL ? [] : [commitEntry.author],
                        updatedAt: commitEntry.timestamp.toISOString(),
