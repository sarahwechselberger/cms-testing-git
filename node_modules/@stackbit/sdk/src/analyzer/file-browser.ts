...
// Type for Octokit instance (dynamically imported at runtime due to ESM-only package)
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore TS1479 - Type-only import, actual import is dynamic
type OctokitModule = typeof import('@octokit/rest');
type Octokit = InstanceType<OctokitModule['Octokit']>;
...
export class GitHubFileBrowserAdapter implements FileBrowserAdapterInterface {
    ...
    private readonly octokitPromise: Promise<Octokit>;
    ...
    constructor(options: GitHubFileBrowserAdapterOptions) {
        ...
        this.octokitPromise = this.initOctokit(options.auth);
    }

    private async initOctokit(auth?: string): Promise<Octokit> {
        const { Octokit: OctokitClass } = await import('@octokit/rest');
        this.octokit = new OctokitClass({ auth });
        return this.octokit;
    }
    ...
    async listFiles(listFilesOptions: ListFilesOptions): Promise<FileResult[]> {
        ...
        if (!treeResponse.data.truncated) {
            ...
            tree = tree.filter((node: OctokitTreeNode) => {
        ...
        // TODO: order files alphabetically so both Git and FileSystem adapters will result same analyze results
        return tree.map((node: OctokitTreeNode) => ({
            filePath: node.path!,
